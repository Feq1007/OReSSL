...
def split_init_data(dataset, init_size, label_ratio, imb_ratio, imb_type='long', datatype='realworld'):
    data_path = f"data/benchmark/{datatype}/{dataset}/{dataset}.csv"
    data = pd.read_csv(data_path, dtype=float)
    ...
    print(r"done")
    np.save(f'./data/init/{dataset}.npy', init_data, allow_pickle=False)
    np.save(f'./data/eval/{dataset}', eval_data, allow_pickle=False)


if __name__ == "__main__":
    opt = cfg.get_options()
    split_init_data(opt.dataset, opt.init_size, opt.label_ratio, opt.imb_ratio, opt.imb_type, opt.datatype)
