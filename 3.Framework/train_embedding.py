...
def main(opt):
    ...
    print(f'train size:{len(train_dataset)}, evaluate size:{len(eval_dataset)}')
    ...
    for i in range(opt.eval_times):
        ...
        nmi, recall, cur_acc = validate(eval_dataloader, models, device, opt)
        print(
            '{i}-th: Recall@1, 2, 4, 8: {recall[0]:.3f}, {recall[1]:.3f}, {recall[2]:.3f}, {recall[3]:.3f}; NMI: {nmi:.3f}; accuracy: {acc:.3f} \n'
                .format(i=i + 1, recall=recall, nmi=nmi, acc=acc))
        if cur_acc > acc:
...
def train(epoch, train_loader, models, criterions, miner, optimizers, device, opt):
    ...
    for i, (inputs, target, _) in enumerate(train_loader):
        ...
        # plot progress
        bar.suffix = 'Epoch: {epoch:3d} | ({batch}/{size}) Data: {data:.3f}s | Batch: {bt:.3f}s | Total: {total:} | ETA: {eta:} | ' \
                     'Loss: {loss:.4f} | Acc: {acc:.3f} '.format(
            batch=i + 1,
...
def stream_test(opt):
    ...
    y = data[:, -2:].astype(np.int64)
    dataset = DataSet(x, y)
    dataloader = DataLoader(dataset, batch_size=opt.batch_size * 10, shuffle=False, num_workers=2)
...
def main_abc(args):
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        test_loss, test_acc, testclassacc = validate_imbalance(eval_dataloader, ema_model, criterion, mode='Test Stats ', class_num=class_num)
        print(testclassacc)
...
def train_imbalance(labeled_trainloader, unlabeled_trainloader, model, optimizer, ema_optimizer, criterion, epoch, ir2,
    ...
    for batch_idx in range(args.val_iteration):
        ...
        with torch.no_grad():
            ...
            # Tracking the empirical distribution on the unlabeled samples (ReMixMatch): only consider the 128 former
            real_batch_idx = batch_idx
            if type(emp_distb_u) == int:
                emp_distb_u = p.mean(0, keepdim=True)
            elif real_batch_idx == 0:
                emp_distb_u = p.mean(0, keepdim=True)
            elif real_batch_idx // args.align_length == 0:
                emp_distb_u = torch.cat([emp_distb_u, p.mean(0, keepdim=True)], 0)
            else:
                emp_distb_u = emp_distb_u[-(args.align_length - 1):]
                emp_distb_u = torch.cat([emp_distb_u, p.mean(0, keepdim=True)], 0)
        ...
        if args.dataset != 'cifar100' and args.imb_type == 'step':
            maskforbalance = torch.bernoulli(torch.sum(targets_x2 * ir22.to(device), dim=1).detach())
        else:
            maskforbalance = torch.bernoulli(torch.sum(targets_x2 * ir2.to(device), dim=1).detach())
        ...
        # maskforbalanceu = torch.bernoulli(torch.sum(label_u.to(device) * ir2.to(device), dim=1).detach())
        ...
        abcloss1 = -torch.mean(
            select_mask * maskforbalanceu * torch.sum(torch.log(logitsu2) * logitsu1.to(device).detach(), dim=1))
        abcloss2 = -torch.mean(
            select_mask * maskforbalanceu * torch.sum(torch.log(logitsu3) * logitsu1.to(device).detach(), dim=1))
        Lx, Lu, w = criterion(logits_x, mixed_target[:batch_size], logits_u, mixed_target[batch_size:],
                              args.lambda_u, epoch + batch_idx / args.val_iteration, args.epochs)
        ...
        loss = Lx + w * Lu + args.w_ent * Le * linear_rampup(
            epoch + batch_idx / args.val_iteration, args.epochs) + totalabcloss
...
class SemiLoss(object):
    def __call__(self, outputs_x, targets_x, outputs_u, targets_u, lambda_u, epoch, epochs):
...
def validate_imbalance(val_loader, model, criterion, mode, class_num):
    batch_time = AverageMeter()
    data_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
...
#                 top5=top5.avg,
    ...
    rep = classification_report(label_true, label_predict, digits=4)
    print(rep)
