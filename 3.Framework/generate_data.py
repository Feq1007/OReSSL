import config.data_config as cfg
import numpy as np
import pandas as pd
...
def sample(labels, class_info, init_size):
    ...
    """
    init_nums = np.array(class_info.iloc[:,-1], dtype=float) / np.sum(class_info.iloc[:,-1]) * init_size + 1 # plus one: make sure every class are included
